...
const Menus = () => {
  ...
  const getAllData = async (_prams) => {
    ...
    const dataRes = await getPagingSlide(params);
    const data =
      dataRes?.data &&
  ...
  const onFinish = async (data) => {
    ...
    if (!data.id) {
      //Save
      const dataRes = await insertSlide(dataReq);
      if (dataRes.status === 1) {
      ...
      //Update
      const dataRes = await updateSlide(data.id, dataReq);
  ...
  const onDelete = async (key) => {
    const dataRes = await deleteSlide(key);
    dataRes.status === 1
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Slide" pageTitle="Management Slides" />
